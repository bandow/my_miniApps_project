...
var allpacks = {
    ...
    "taobao":[
        ...
        },
        {
            ...
            "price":"套餐价 20万",
            "avatar":"../../images/ic_go_04.png",
            "apply":"站内大曝光，月过百万不是梦    适用：店铺促销、提升业绩",
            "contents":[
                {
                    "name":"淘宝头条",
                    "desc":"30篇内容",
        ...
        },
        {
            ...
            "price":"套餐价 45万",
            "avatar":"../../images/ic_go_04.png",
            "apply":"迈向一线流量店铺，让小二关注你    适用：瓶颈期的二线店铺",
            "contents":[
                {
                    "name":"淘宝头条",
                    "desc":"70篇内容",
...
Page({
  ...
  onLoad:function(options){
    var title = "加载数据失败", currpack = null, index = options.index;

    if(options.type == this.data.PACKTYPE.TECH){
        currpack = allpacks.tech[index];
    }else if(options.type == this.data.PACKTYPE.STAR){
        currpack = allpacks.star[index];
    }else if(options.type == this.data.PACKTYPE.ZHIHU){
        currpack = allpacks.zhihu[index];
    }else if(options.type == this.data.PACKTYPE.TAOBAO){
        currpack = allpacks.taobao[index];
    }else if(options.type == this.data.PACKTYPE.FOREIGN){
        currpack = allpacks.foreign[index];
    }
    if(currpack && currpack.contents){
        title = "共"+ currpack.contents.length +"条资源",
        this.setData({ptype:options.type,index:options.index,packitem:currpack});
    }
    wx.showShareMenu();
